...
function Dashboard() {
  ...
  const { data: triagemStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const horas = (concluidas ?? [])
        .map(
          (r: any) =>
            (new Date(r.triagem_concluida_em).getTime() - new Date(r.created_at).getTime()) /
            3600000,
        )
      ...
      const { data: maisAntiga } = await supabase
        ...
        .limit(1)
        .maybeSingle();
      const esperaH = maisAntiga
        ? (Date.now() - new Date((maisAntiga as any).created_at).getTime()) / 3600000
        : 0;
      return {
        pendentes: pendentes ?? 0,
        mediaHoras: media,
        esperaMaiorHoras: esperaH,
        amostra: horas.length,
      };
    },
  ...
  const allEnriched = useMemo(() => [...enriched].sort(sortProtocolosPorNumero), [enriched]);
  ...
  const refDateAtraso = useMemo(() => (mes === "all" ? new Date() : endOfMonthOrNow(mes)), [mes]);
  ...
  const prazoMedio = useMemo(() => {
    ...
    const soma = fechados.reduce(
      (acc, p) =>
        acc + Math.max(0, differenceInDays(new Date(p.data_conclusao!), new Date(p.data_abertura))),
      0,
    );
  ...
  const reclamacoesSaude = useMemo(() => {
    ...
    const recs = filtrados.filter(
      (p) =>
        ...
        saudeIds.includes(p.secretaria_id) &&
  ...
  const insights = useMemo(() => {
    ...
    if (topAssuntos.length > 0 && total > 0) {
      ...
      list.push({
        ...
        text: (
          <>
            <span className="font-semibold">{top.nome}</span> representa{" "}
            <span className="font-semibold">{pct}%</span> das manifestações do período.
          </>
        ),
      });
    }
    if (porSecretaria.length > 0 && total > 0) {
      ...
      list.push({
        ...
        text: (
          <>
            Secretaria de <span className="font-semibold">{topSec.full}</span> concentra{" "}
            <span className="font-semibold">
              {topSec.qtd} protocolos ({pct}%)
            </span>{" "}
            do total.
          </>
        ),
    ...
    list.push({
      ...
      text:
        vencidos.length === 0 ? (
          ...
          <>
            <span className="font-semibold">{vencidos.length}</span> protocolo(s) em atraso requerem
            atenção imediata.
          </>
        ),
    });
      ...
      list.push({
        ...
        text: abaixo ? (
          <>
            ...
            {prazoMedio} dias vs {meta} dias).
          </>
        ) : (
          <>
            Tempo médio de resposta <span className="font-semibold">{diff}% acima da meta</span> de{" "}
            {meta} dias ({prazoMedio} dias atual).
          </>
        ),
      });
    }
    ...
    if (total > 0) {
      ...
      list.push({
        ...
        text: (
          <>
            <span className="font-semibold">{pctConc}%</span> dos protocolos estão concluídos. Meta:{" "}
            {metaConclusao}%.
          </>
        ),
      });
    ...
    if (ativos.length > 0) {
      ...
      list.push({
        ...
        text: (
          <>
            <span className="font-semibold">{pctNoPrazo}%</span> dos protocolos em andamento estão
            dentro do prazo.
          </>
        ),
      });
    ...
    if (reclamacoes + elogios > 0) {
      ...
      list.push({
        ...
        text: (
          <>
            <span className="font-semibold">{ratio}%</span> de aprovação entre elogios e reclamações
            ({elogios} elogios vs {reclamacoes} reclamações).
          </>
        ),
      });
    }
    ...
    if (total > 0) {
      ...
      list.push({
        ...
        text: (
          <>
            <span className="font-semibold">{pctGeo}%</span> das manifestações incluem localização
            geográfica.
          </>
        ),
      });
      ...
      list.push({
        ...
        text: (
          <>
            ...
            manifestações em relação ao mês anterior ({penultimo.mes}: {penultimo.total} →{" "}
            {ultimo.mes}: {ultimo.total}).
          </>
        ),
      });
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Visão geral das manifestações da Ouvidoria —{" "}
            {mes === "all" ? "todos os meses" : formatMonthLabel(mes)}
          </p>
        </div>
        <SyncBadge />
      ...
      {/* KPI Row */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3"></div>
        ...
        <CardContent className="py-3 px-4 flex items-center gap-4 flex-wrap">
          ...
          <div className="border-l pl-4">
            ...
            <div className="text-[10px] text-muted-foreground">
              {triagemStats?.amostra ?? 0} triadas
            </div>
          </div>
          <div className="border-l pl-4">
            <div className="text-xs text-muted-foreground">Mais antiga na fila</div>
            <div
              className={`text-lg font-semibold ${(triagemStats?.esperaMaiorHoras ?? 0) > 24 ? "text-red-600" : ""}`}
            >
              {triagemStats && triagemStats.pendentes > 0
                ? fmtDuracao(triagemStats.esperaMaiorHoras)
                : "—"}
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <KpiCard
          ...
          onClick={() =>
            openDrill(
              "Protocolos concluídos (prazo médio)",
              (p) => p.status === "concluido" && !!p.data_conclusao,
            )
          }
        />
        <KpiCard
            ...
            <div className="h-[260px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={evolucao} margin={{ top: 20, right: 20, bottom: 0, left: -20 }}>
                  ...
                  />
                  <Tooltip
                    content={<ChartTooltipContent unit="manifestações" />}
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      openDrill(
                        `Categoria: ${d.name}`,
                        (p) => (p.categoria ?? "outros") === (cat?.value ?? "outros"),
                      )
                    }
                ...
                return topAssuntos.map((a, idx) => (
                  <button
                    ...
                    onClick={() =>
                      openDrill(
                        `Assunto: ${a.nome}`,
                        (p) => (p.assunto ?? "Outros").slice(0, 40) === a.nome,
                      )
                    }
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-medium truncate" title={a.nome}>
                        {a.nome}
                      </p>
                    ...
                    <span className="text-xs font-bold tabular-nums text-foreground min-w-[20px] text-right">
                      {a.qtd}
                    </span>
                  </button>
                ));
                ...
                >
                  ...
                  <Bar
                    ...
                    onClick={(d: any) => {
                      ...
                      if (sec)
                        openDrill(`Secretaria: ${sec.nome}`, (p) => p.secretaria_id === sec.id);
                    }}
                  />
                </BarChart>
              </ResponsiveContainer>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="space-y-1.5">
                  {reclamacoesSaude.length === 0 && (
                    <p className="text-xs text-muted-foreground">Sem reclamações.</p>
                  )}
                  {(() => {
                    const max = Math.max(1, ...reclamacoesSaude.map((r) => r.qtd));
                    const saudeIds = secretarias
                      .filter((s) => /sa[uú]de/i.test(s.nome))
                      .map((s) => s.id);
                    return reclamacoesSaude.map((r) => (
                      ...
                      >
                        <span className="text-[11px] truncate flex-1" title={r.nome}>
                          {r.nome}
                        </span>
                        <div className="h-2 w-16 rounded-sm bg-muted overflow-hidden">
                          <div
                            className="h-full bg-orange-500"
                            style={{ width: `${(r.qtd / max) * 100}%` }}
                          />
                        </div>
                    ...
                    return porUnidade.map((u) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full flex items-center gap-2 rounded px-1 py-0.5 hover:bg-muted/60 transition"
                        >
                          <span className="text-[11px] truncate flex-1" title={u.nome}>
                            {u.nome}
                          </span>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4">
              ...
              <Gauge
                ...
                meta="Meta: 0"
                good={vencidos.length === 0}
                max={Math.max(20, vencidos.length)}
                invert
              />
            </div>
          </CardContent>
              ...
              <div className="space-y-1.5">
                {porRegiao.slice(0, 5).map((r, i) => (
                  <RegiaoRow
                    ...
                    nome={r.nome}
                    qtd={r.qtd}
                    max={porRegiao[0]?.qtd ?? 1}
                    onClick={() =>
                      openDrill(
                        `Local: ${r.nome}`,
                        (p) => ((p as any).locais?.nome ?? "Sem local") === r.nome,
                      )
                    }
                  />
                ))}
...
function KpiCard({
  ...
  const tones: Record<string, { bg: string; fg: string; ring: string; grad: string }> = {
    primary: {
      ...
      grad: "from-primary/10 via-transparent to-transparent",
    },
    info: {
      bg: "bg-amber-500/10",
      fg: "text-amber-600",
      ring: "ring-amber-500/20",
      grad: "from-amber-500/10 via-transparent to-transparent",
    },
    success: {
    ...
    destructive: {
      ...
      grad: "from-destructive/10 via-transparent to-transparent",
    },
    violet: {
      bg: "bg-violet-500/10",
      fg: "text-violet-600",
    ...
    emerald: {
      ...
      grad: "from-teal-500/10 via-transparent to-transparent",
    },
  };
  const t = tones[tone];
  return (
    ...
    >
      <CardContent className="p-4 relative">
        <div className="flex items-center gap-3">
          <div
            className={`h-11 w-11 rounded-xl ${t.bg} ${t.fg} flex items-center justify-center shrink-0 shadow-sm`}
          >
        ...
        {hint && (
          <p
            className={`mt-2 text-[11px] flex items-center gap-1 ${hintTrend === "up" ? "text-emerald-600" : hintTrend === "down" ? "text-destructive" : "text-muted-foreground"}`}
          >
            {hint}
            {hintTrend === "up" && <TrendingUp className="h-3 w-3" />}
          </p>
        )}
      </CardContent>
    </Card>
    ...
    <div className="flex flex-col items-center text-center">
      ...
      <div className="relative w-28 h-16">
        <svg viewBox="0 0 100 60" className="w-full h-full">
          <path
            d={`M 8 56 A ${radius} ${radius} 0 0 1 92 56`}
...
function RegiaoRow({
  ...
  const color =
    ratio > 0.66 ? "hsl(0 84% 60%)" : ratio > 0.33 ? "hsl(48 96% 53%)" : "hsl(142 71% 45%)";
  const textColor = ratio > 0.33 && ratio <= 0.66 ? "#000" : "#fff";
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full flex items-center gap-2 rounded px-1 py-0.5 hover:bg-muted/60 transition text-left"
    >
      <span
      ...
      <span className="text-xs font-medium truncate flex-1" title={nome}>
        {nome}
      </span>
    </button>
  );
...
function InsightCard({
  ...
  return (
    <div
      className={`flex items-start gap-3 rounded-xl border bg-card/50 ring-1 ${t.ring} p-3 transition hover:bg-card`}
    >
      <div
        className={`h-9 w-9 shrink-0 rounded-lg ${t.bg} ${t.fg} flex items-center justify-center`}
      >
    ...
    >
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Clique em um protocolo para abrir os detalhes.</DialogDescription>
        </DialogHeader>
                ...
                <li key={p.id}>
                  ...
                  >
                    ...
                    {typeof p?._s?.dias === "number" && p._s.dias < 0 && (
                      <Badge
                        variant="outline"
                        className="text-[10px] shrink-0 bg-destructive/10 text-destructive border-destructive/30"
                      >
